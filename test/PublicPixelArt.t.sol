...
contract PublicPixelArtTest is Test {
    ...
    function test_DrawPixelsBatch_MixedNewAndExistingPixels() public {
        ...
        assertEq(retrievedColor1[0], 255);
        assertEq(retrievedColor1[1], 0);
        assertEq(retrievedColor1[2], 0);

        uint8[] memory retrievedColor2 = pixelArt.getPixelColor(30, 40);
        assertEq(retrievedColor2[0], 0);
        assertEq(retrievedColor2[1], 0);
        assertEq(retrievedColor2[2], 255);
    ...
    function test_PixelChangedEvent() public {
        uint256 expectedKey = pixelArt.getPixelKey(10, 20);
        uint8[] memory color = new uint8[](3);
        color[0] = 255;
        color[1] = 0;
        color[2] = 0; // 红色
